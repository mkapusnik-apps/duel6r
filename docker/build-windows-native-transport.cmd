...
cmake --build C:\workspace\build-windows-native-transport --config Release --target duel6r-session-transport-tests duel6r-network-trust-policy-tests duel6r-admission-compatibility-tests duel6r-server duel6r-host-supervisor
if errorlevel 1 exit /b %errorlevel%

ctest --test-dir C:\workspace\build-windows-native-transport -C Release --output-on-failure -R "^(duel6r-network-trust-policy-tests|duel6r-session-transport-(tests|process-tests)|duel6r-admission-(compatibility-tests|process-tests))$"
exit /b %errorlevel%
