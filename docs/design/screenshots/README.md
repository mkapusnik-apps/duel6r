...
**Current conformance: Pending for all 16 entries. No new implementation artifacts have been supplied or assessed.** The approved [shared visual baseline](../../design.md#unified-network-presentation), [screen index](../README.md#screen-specifications), and linked wireframes define assessment. Existing functional contracts remain unchanged. NET-02-P, NET-05-S, and NET-05-R add explicit coverage for existing materially different layouts, not new functional states. All other screen and wireframe identities remain unchanged.
...
Every row is Pending. Paths are repository-relative. A stated test endpoint is an input for an authorized owned session, not a public service or a claim of reachability.
...
| NET-02-P / NET-02 / [NET-02-P](../wireframes/NET-02/NET-02-P.svg) | From valid NET-02, activate Start session; capture the actual pending startup before confirmed readiness. | Starting session and timing help; Cancel focused; no editable setup, second Start, or success claim. | 1280 × 900 | `docs/design/screenshots/NET-02/NET-02-P.png` |
...
| SS-018 / NET-04 / [NET-04](../../screens/wireframes/network-lobby.md) | Host plus two guests, two slots each; valid Deathmatch/three-round setup; leave one guest unready; make only the authorized test directory unavailable while transport remains active. | Host lobby; six players/three participants; publication failure and Retry publication; separate participant columns; disabled Start reason; framed settings/actions. | 1280 × 900 | `docs/design/screenshots/NET-04/NET-04.png` |
| SS-025 / NET-04 / [NET-04-R](../wireframes/NET-04/NET-04-R.svg) | Complete the NET-06 representative's actual three-round match; host Return to lobby in the same session; retain the result and confirmed directory state. | Completed retained result; current membership distinct from outcome history; separate fixed outcome labels, result scroll feedback, readiness, and footer. | 1280 × 900 | `docs/design/screenshots/NET-04/NET-04-R.png` |
| SS-019 / NET-05 / [NET-05](../../screens/wireframes/network-match.md) | Host and two guests with two slots each; real Deathmatch on shipped duel_16, one round; guest obtains actual Invisibility with held weapon; briefly interrupt owned forwarding and capture after complete updates resume while degraded status remains. | Guest live/degraded six-player arena; unchanged orientation and gameplay HUD; exact degraded text; framed Leave session action. Record actual mirror and background state. | 1280 × 900 | `docs/screenshots/NET-05/six-player-lan-degraded-1280x900.png` |
...
| NET-05-S / NET-05 / [NET-05-S](../wireframes/NET-05/NET-05-S.svg) | Host plus guest in actual Deathmatch, three-round limit; before the first round has a winner, press Tab once. | Real pre-winner authoritative score overlay; Session only/In progress and actual available values; fixed labels/body/scroll controls; no fabricated completed rows. | 1280 × 900 | `docs/design/screenshots/NET-05/NET-05-S.png` |
| NET-05-R / NET-05 / [NET-05-R](../wireframes/NET-05/NET-05-R.svg) | Same three-round Deathmatch; finish round one by normal play; capture host after the first active second and before automatic advance, without activating Advance round. | Frozen non-final round result; Rounds 1\|3, actual outcome, positive next-round countdown, ranking, framed Advance round and End session. | 1280 × 900 | `docs/design/screenshots/NET-05/NET-05-R.png` |
| SS-020 / NET-06 / [NET-06](../../screens/wireframes/network-summary.md) | Host plus two guests with two slots each; complete all three Deathmatch rounds through actual gameplay; capture host before Return to lobby. | Completed final summary in existing menu context; Session only and no-persistence copy; separate match/last-round outcomes; actual outcome rows; fixed scroll feedback and host actions. | 1280 × 900 | `docs/screenshots/NET-06/final-three-round-summary-1280x900.png` |
| SS-021 / NET-07 / [NET-07](../../screens/wireframes/network-reconnect.md) | Interrupt an authorized owned guest connection during active play; capture at displayed 24 seconds remaining; restore before expiry after capture. | Actual reconnect; Last confirmed state context; countdown/reservation/continuation text; grey framed panel and focused Leave session. | 1280 × 900 | `docs/screenshots/NET-07/reconnecting-24s-1280x900.png` |
...
| NET-10 / NET-10 / [NET-10](../wireframes/NET-10/NET-10.svg) | Network → Browse sessions; provision actual owned listings for open lobby, protected open round one, full session, and closed started session; select protected open row. | NET-10-results; white inset table, grey column headings, blue selected row plus non-color focus/selection; details; Page 1 without invented total; all footer buttons. | 1280 × 900 | `docs/design/screenshots/NET-10/NET-10.png` |

NET-06 and NET-04-R use an ordinary completed Deathmatch for default style coverage. The historical fourteen-winner Predator packet is not relabeled as this new state; maximum-result checks remain required below. NET-05-S must show whatever data the actual pre-winner state provides, including an empty result body when applicable. Capture must not manufacture populated rows to resemble the diagram. NET-05-R records the actual positive countdown rather than requiring a particular frozen-second value.
...
NET-02-P must use actual host startup. A short capture window is not permission to inject a runtime state, add a production delay, or show a mock screen. Team must approve any controlled startup boundary needed for reproducibility. NET-03 permits the existing owned pending-listener setup because the actual client is genuinely awaiting admission. It is not successful-admission evidence. No network infrastructure change is implied.
...
| Input preservation | Keyboard/controller traversal, reverse traversal, pointer bounds, existing press-versus-release activation, repeat behavior, disabled exclusion, focus retention, and opening-input protection. |
...
| NET-04 / NET-04-R | Host/guest permissions; 15 participants/players; Team settings shown versus absent; readiness clearing; publication retry; completed/interrupted result; departed identities; full 64 UTF-8-byte names; vertical and horizontal scroll extremes. |
| NET-05 / NET-05-C/S/R | Normal/degraded/resynchronizing status; all three modes; first-second active versus frozen phase; guest absence of Advance; long outcome and maximum ranking; Tab toggling; no hidden action activation; confirmation over arena/lobby/summary/reconnect; first-round arrival. |
...
| NET-10 | Loading, results, empty, stale, unavailable; full/closed/protected selection; disabled reasons; both page directions and current page; removed selection; refresh focus retention; long endpoints; independent Direct connect. |
...
UX will assess structure against the existing or new owning wireframe, component appearance against `docs/design.md`, and interaction presentation against the owning UX section. Wireframes are low fidelity; pixel matching is not required. Each row remains Pending until supplied evidence is inspected. Exact-hash integration without presentation-source or assessed-content changes does not require another assessment. Changed shared presentation invalidates every consuming representative; unrelated local evidence is retained unless its presentation changes.
