...
| Invalid manifest (including unsafe/unreadable content) | `gameplay-content-manifest-invalid` | `Gameplay content manifest is invalid. Use the host's exact supported gameplay content.` Verify a clean package and safe local filesystem layout. |
...
The UI may require restart for invalid local content; follow its disabled-retry explanation. No new protocol error detects installation history or a downgrade. Unsupported procedures are documentation boundaries; runtime rejects only the incompatibilities it can observe under the existing contract.
