...
It is not safe or supported on a hostile multi-user machine, the public Internet, a public VPN, a relay, NAT or port-forwarded exposure, or any network where an untrusted party can reach the port. Default bind is `127.0.0.1`; the exact `localhost` name is also accepted only when it resolves to loopback. LAN hosting requires an explicit `10/8`, `172.16/12`, or `192.168/16` IPv4 literal that operating-system interface enumeration confirms is assigned to a local interface. Listener startup rejects every other hostname, unassigned private address, wildcard/unspecified address, public address, multicast, limited broadcast, and link-local address before creating a listener. Address scope is determined by the full IPv4 range: a loopback or RFC1918 address ending in `.255` is not presumed to be a directed broadcast because that depends on the interface prefix; it is usable as a listener only when it is the assigned local interface address. Guest endpoint hostnames are ASCII and at most 253 bytes; resolution may return at most 64 IPv4 addresses and connection attempts retain loopback or RFC1918 results, including `.255` destinations, because the remote interface prefix is unknown.
...
An unsupported or unassigned listener address emits only the applicable rejection line and exits before constructing a listener. In that fixed copy, “private LAN address” means a private address assigned to the host machine; the copy remains non-disclosing and does not reveal interface details. Normal `duel6r` startup and local-only Play still start no network service.
