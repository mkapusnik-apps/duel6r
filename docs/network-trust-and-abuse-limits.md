...
Malformed data fails closed. Path, participant-name, and reason validators can be applied before allocation, logging, manifest comparison, or authority changes. Guest profile metadata is data only and cannot select or load a local file or script.
...
The #36 handoff provides a memory-only opaque credential generated by the operating system CSPRNG (`BCryptGenRandom` on Windows and `getrandom` on Linux). It is 128 bits, scoped to one session, participant, and reservation, expires after 30 seconds, and uses constant-time comparison. Every consume attempt invalidates the credential before any caller can restore identity, preventing replay and oracle-style disclosure. A later disconnect creates a new reservation and credential. This issue does not implement credential exchange, persistence, command-line transfer, logging, reconnect attempts, or state restoration.
...
- #36 owns disconnect reservation lifecycle, secure credential exchange over the approved future session channel, reconnect attempt state, invalidate-before-restore sequencing, and rotation after later disconnect.
