...
It is not safe or supported on a hostile multi-user machine, the public Internet, a public VPN, a relay, NAT or port-forwarded exposure, or any network where an untrusted party can reach the port. Default bind is `127.0.0.1`; the exact `localhost` name is also accepted only when it resolves to an allowed assigned loopback address. LAN hosting requires an explicit `10/8`, `172.16/12`, or `192.168/16` IPv4 literal that operating-system interface enumeration confirms is assigned to a local interface and is unicast for that interface prefix. Linux supplies the netmask and, where applicable, `ifa_broadaddr`; Windows supplies `OnLinkPrefixLength`. Listener startup rejects every other hostname, unassigned private address, network address, directed or limited broadcast address, wildcard/unspecified address, public address, multicast, and link-local address before creating a listener. Prefixes `/0` through `/30` have network and broadcast endpoints, both rejected. RFC 3021 `/31` interfaces have no network/broadcast pair, and `/32` is treated as a host route. Address scope is still determined by the full IPv4 range: `.255` is not presumed broadcast without prefix metadata, so assigned `10.0.0.255/16` is valid while assigned `192.168.1.255/24` is not. Guest endpoint hostnames are ASCII and at most 253 bytes; resolution may return at most 64 IPv4 addresses and connection attempts retain loopback or RFC1918 results, including `.255` destinations, because the remote interface prefix is unknown.
...
- **Admission:** a transport connection has no committed participant identity, player slot, readiness, or host authority. It may submit exactly one bounded initial request within three seconds. A successful compatibility decision creates only a private provisional reservation; one matching acceptance is due within three seconds before atomic commit.
...
| Offer acceptance deadline | 3 seconds |
| Concurrent manifest validations | 2 |

Pending accounting is keyed by source IPv4 and is released when admission commits or the connection closes. Rejected offers and offers interrupted by send failure, invalid or missing acceptance, cancellation, timeout, or pre-admission close roll back all provisional participant, player-slot, ownership, and authority state. Provisional identity values remain burned and cannot be reassigned within the session. The policy deliberately permits sequential and concurrently admitted participants behind one trusted IPv4 address. Limits are temporary controls only: there is no persistent ban or source denylist.
...
Bandwidth and non-input action limits use token buckets. Token-bucket rates, capacities, and consumption requests must be finite and non-negative; invalid constructor values are rejected and invalid consumption never mutates the bucket. Input limits use bounded one-second counters because no additional input burst was approved. At most one input may be applied for one player slot in one simulation tick. Two over-limit windows close only the offender when their monotonic one-second window indices differ by exactly one. An empty or skipped window resets the sequence, as does an injected clock moving backward; index comparison is overflow-safe. The policy primitives are independent of gameplay; #33 and the authoritative simulation must consume them when those message/application paths exist.

## Authorization and outcomes

The authorization primitive models a locally created host, immutable connection-to-participant bindings, immutable participant-owned player slots, host-only actions, and guest-own readiness/proposal/leave/input actions. Host identity and ownership are initialized before listener readiness. Guest binding and ownership become visible together only when the matching admission acceptance commits. Unauthorized authority action uses `session-policy-violation`, the fixed copy `Connection ended.`, and requires closing only the offender. No lobby or simulation behavior is implemented here.
...
The #36 handoff provides a memory-only opaque credential generated by the operating system CSPRNG (`BCryptGenRandom` on Windows and `getrandom` on Linux). It is 128 bits, scoped to one session, participant, and reservation, expires after 30 seconds, and uses constant-time credential comparison. Generation retries at most four times and fails closed if the random source fails or repeatedly returns all-zero bytes. An all-zero credential is never made active, returned, or accepted. Credential objects own their bytes through RAII: storage and every controlled copy or move are erased with `SecureZeroMemory` on Windows, `explicit_bzero` on supported Linux, or a volatile-write fallback before release. Move operations erase the source. Reservation invalidation and destruction, successful consume, expiry, cancellation, participant removal, session end, replacement, failed generation candidates, and old replacement values all use the same non-optimizable erase path.
