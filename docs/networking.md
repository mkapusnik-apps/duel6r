...
- Startup/connect cancellation closes pending sockets and targets cleanup within one second. The operating-system hostname resolver is synchronous; cancellation is observed immediately after it returns, so resolver implementations that block inside `getaddrinfo` can delay worker cleanup. The total attempt still cannot publish Connected after cancellation or at/after its deadline.

The transport API is `source/network/SessionTransport.h`. It deliberately exposes opaque frames rather than release/content compatibility, admission, client IDs, reconnect reservations, or simulation messages; those remain downstream policy.
...
CMake builds `duel6r-network-scaffold`, including the transport API, and `duel6r-server` independently from the normal game executable. The transport uses standard OS sockets (`ws2_32` on Windows and POSIX sockets/threads on Linux) with no new third-party dependency. Existing Linux and Windows runtime bundle scripts place the explicitly invoked server scaffold beside `duel6r` because those are the repository's current coherent binary packaging paths.
