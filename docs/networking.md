...
- Startup/connect cancellation closes pending sockets, stops and joins the transport worker, and releases resolver resources within one second. Linux isolates blocking `getaddrinfo` work in a bounded child resolver process that is killed and synchronously reaped on cancellation or deadline. Windows uses overlapped `GetAddrInfoEx` with its cancellation handle and completion event. Neither path detaches a resolver thread, and neither can publish Ready or Connected after cancellation or at/after the shared deadline.
