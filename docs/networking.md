...
Linux uses a dedicated process group plus a child-installed parent-death signal that terminates that full group and a race-safe parent check. Windows validates the creating parent and creates the child atomically in a kill-on-close Job Object whose active-process count confirms tree cleanup. Both platforms retain the process handle rather than adopting a PID, restrict inherited descriptors or handles to standard null streams and the lifecycle channels, and release the listener, admission state, queues, and process resources during cleanup. Normal shutdown, parent crash, forced termination, and unexpected service failure do not create an intentional host-end notice. The separate `End session` supervisor action records `intentional-host-end` and invokes the one downstream handoff reserved for intentional notice delivery; this scaffold does not claim delivery.
