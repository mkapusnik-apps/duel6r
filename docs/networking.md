...
- Guest Cancel is checked before every host message and immediately before acceptance enqueue and outcome publication. Acceptance performs a final monotonic deadline check and is never queued at or after the guest's original Connect-start plus 10 seconds. At deadline or terminal selection, the connection atomically seals and drains already queued complete frames against the reader thread; authoritative `receivedAt` timestamps preserve a strictly pre-deadline valid rejection or exact confirmation over delayed polling or a later close, while late frames cannot create acceptance or success.
