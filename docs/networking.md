...
- In each direction, at most 256 application frames or 4 MiB of application payload may be queued, whichever limit is reached first. `send` immediately returns `Backpressure` without accepting, dropping, or reordering the new frame. Payloads above 1,048,576 immediately return `PayloadTooLarge`. Up to two zero-payload liveness controls use a separate priority queue, idle connections probe liveness every 2 seconds, and connected sockets require no-delay writes and cap kernel send buffering at 96 KiB so already-submitted application bytes cannot hide controls behind the full 4 MiB application queue. These controls and bounded kernel write-ahead do not weaken application backpressure.
