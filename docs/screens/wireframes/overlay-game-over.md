...
The representative visual table is the final limited four-team variant over the undivided shared arena.
...
The representative state uses a one-round limited Team match with four teams and two nested players per team.
...
The counter backing stays inside the client and remains separate from the score panel by at least 16 px.
...
The score panel moves within the available middle region only as needed when the preferred position does not fit.
