...
| <a id="ss-019"></a>`SS-019` | `NET-05` | [Network match](../screens/wireframes/network-match.md) | Start a LAN Deathmatch after all participants are ready, use local persons with profile appearances that differ from the built-in network default, sustain an applicable network-budget breach until the degraded state appears, and keep complete canonical updates flowing. | Use six living players across connected participants during active play from the host view. | 1280x900 | One undivided arena must preserve the complete level, six players, ranking, round progress, events, and player status. All six players must use the built-in default network player skin rather than a selected-profile appearance. The bottom status region must show `Host`, `LAN session`, `Connected`, `Session only scores`, `Optional scripts disabled`, and exact `Network connection degraded.` without overlap, clipping, truncation, disconnect, pause, reconnect, or host-end copy. | `docs/screenshots/NET-05/six-player-lan-degraded-1280x900.png` | `Planned` for downstream issue #38. Issue #35 supplies the production presentation handoff but no graphical consumer or playable network screen. A supplementary recording, not another canonical screenshot, must provide deterministic animation mapping, temporal correction, and recovery evidence after issue #38 implements the consumer. |
