...
The existing `OVER-02/resumed-round-3-of-5-1280x900.png` artifact is stale because it puts the label in the heading strip and retains duplicate arena progress.
`SS-011` is pending implementation and recapture.
...
- Coverage status: eight implementation artifacts conform. `SS-011` is pending implementation and recapture. Eleven planned entries have no valid current artifacts. Issue #38 must invalidate and replace `SS-001` when it implements the Network footer.
