...
`SS-012` is `Pending` because the current `OVER-03` requirements enlarge and contain the final round counter, add the completion notice to limited Deathmatch, and define readable backing overlap for an oversized score panel.
...
- Current evidence status: `Pending`; the artifact does not establish the current final round-counter size, containment, Deathmatch notice, or oversized-panel overlap requirements.
...
- Pending screenshot entries: 1.
- Screenshots per wireframe: exactly one.
- Retired screenshot matrix entries: `SS-004`, `SS-005`, and `SS-006`.
- Coverage status: nine implemented entries conform, `SS-012` needs replacement, and eleven planned entries have no current artifacts.
