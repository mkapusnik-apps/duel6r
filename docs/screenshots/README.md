...
`SS-001`, `SS-002`, and `SS-013` require replacement captures at 1920 by 1080 from one application session.
...
The approved scaled-background replacement packet is pending capture with this required shared setup:

- Branch: `feature/menu-presentation`.
- Implementation source SHA: to be recorded after the coherent implementation commit.
- Environment: Release, GL4, Lua enabled, Ubuntu 24.04 Docker image, Mesa software rendering, and Xvfb at 1920 by 1080.
- Runtime setup: one application session with four saved people, two selected players, persistent score data, Deathmatch, Assistance on, Quick Liquid on, Burnable Trees on, and Rounds `0`.
- Session workflow: capture `MENU-01`, press F3 or select Clear for `MENU-02`, dismiss with `N`, then open the console for `CONS-01`, without restarting.
- Required diagnostics: record the session-selected asset filename from startup output.
- Runtime asset manifest revision: the commit containing `resources/textures/menu-backgrounds/README.md` and the three eligible assets.
...
- Coverage status: eight unchanged gameplay screenshots conform; three approved menu replacement screenshots are pending.
