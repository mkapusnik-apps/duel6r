...
`SS-012` is `Pending` because the current `OVER-03` requirements enlarge and contain the final round counter and add the completion notice to limited Deathmatch.
...
- Current evidence status: `Pending`; the artifact does not establish the current final round-counter size and containment requirements.
