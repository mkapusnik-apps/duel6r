...
PR #79 replaced the `SS-012` destination without changing its stable screen, wireframe, or screenshot ID.
...
`SS-012` is current and conforms at PR #79 exact head `e5d6be083c7d1bf19caf95fcd9b71a51e31de28c`.
...
The current OVER-03 final Team game-summary replacement has this provenance:

- Pull request: `#79`.
- Implementation branch: `feature/game-over-round-counter-size`.
- Production implementation source SHA: `fd439c77f9fe36373960d17232cb16bf1a5df15b`.
- Screenshot artifact commit and exact pull request head: `e5d6be083c7d1bf19caf95fcd9b71a51e31de28c`.
- Capture time: `2026-09-07T17:13:59Z`.
- Environment: Release, GL4, Ubuntu 24.04 Docker, Mesa software rendering, and Xvfb.
- Viewport: 1280 by 900 px.
- Workflow and state: complete a one-round local four-team Team deathmatch with 15 players, 19 score rows, one winning team, final counter `Rounds: 1| 1`, three Team separators, and a separate `End of Game` notice; capture the visible final summary and curtain.
- Artifact: `OVER-03/shared-game-over-1280x900.png`.
- SHA-256: `8524cf0ab748a5f29f659ebb764b43441079cf7826fd1c43d9cbd29d4ecc8107`.
- Visual assessment: the top-centered `Rounds: 1| 1` counter and the `SCORE` heading have matching visible character heights. The black counter backing contains the complete text and remains separate from the score panel. The artifact shows all 19 score rows in four contiguous Team groups and three separator bands that match the non-final Team geometry. Each band contains a centered 2 px rule across the score-table width with clear space above and below. The centered bottom notice uses exact white 32 px text `End of Game` on a solid blue surface with the required bottom inset and a visible clear gap from the complete score panel. No counter, score heading, row, value, panel content, or notice is clipped, covered, replaced, or reduced.
- Hierarchy and usability assessment: the larger counter is readable and consistent with the `SCORE` heading without competing with the primary score table. The separate bottom notice gives the final state a clear completion cue without obscuring the result table.
...
| <a id="ss-012"></a>`SS-012` | `OVER-03` | [Game over](../screens/wireframes/overlay-game-over.md) | Start a one-round local four-team Team deathmatch and finish its only round. | Use 15 players in 19 score rows across Alpha, Bravo, Charlie, and Delta. Use one winning team. Capture while `Rounds: 1| 1`, the final summary, the curtain, and the separate `End of Game` notice are visible. | 1280x900 | The final Team outcome, score panel, dark red curtain, and final round progress must appear over one undivided arena. The top-centered final round counter must use the same visible character height as `SCORE`. Its backing must contain the text without clipping and must remain separate from the score panel. Each team row must touch its nested player rows. Each of the three adjacent Team boundaries must match the 8 px band and centered 2 px rule used in non-final `OVER-02`. A horizontally centered blue notice at the bottom must show exact white text `End of Game`. The notice must keep its 16 px bottom inset, remain separate from the panel by at least 16 px, and must not obscure any score content. | `docs/screenshots/OVER-03/shared-game-over-1280x900.png` | PR #79 replacement at exact head `e5d6be083c7d1bf19caf95fcd9b71a51e31de28c`. Counter-to-heading size parity, top-center placement, backing fit, 19 complete score rows, four contiguous Team groups, three matching separator bands, and the separate centered bottom notice conform. `Conforms`. |
