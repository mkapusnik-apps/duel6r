...
All 21 representative entries are implemented, current, and conform.
The 13 PR #83 artifacts conform at source head `e70a057819c97100b083c3cdaae5dc24566435cd`.
...
`SS-019` uses the degraded-network variant captured and accepted for issue #38.
...
| <a id="ss-018"></a>`SS-018` | `NET-04` | [Lobby](../screens/wireframes/network-lobby.md) | Host a session, admit two guests with two ordered slots each, and keep one guest unready after a permitted person or control edit. | Host waiting with 3 participants, 6 fixed owned slots, and one named unready guest. | 1920x1080 | Separate Role, Connection, Readiness, ownership, person, and control values are visible. The current participant can edit person and control only on existing owned slots. Host roster-order controls are visible. No Add, Remove, or Transfer player-slot action or target appears. Disabled Start and `Waiting for <participant> to be ready` show that the edit cleared readiness. No profile UI or cosmetic warning appears. | `docs/screenshots/NET-04/host-waiting-one-unready-1920x1080.png` | PR #83 accepted at `e70a057819c97100b083c3cdaae5dc24566435cd`; SHA-256 `9c3742da66418145baf41e3d65a9cedacc336b2a95cb8373e2d8818a6e0cbf8e`. `Conforms`. |
| <a id="ss-019"></a>`SS-019` | `NET-05` | [Network match](../screens/wireframes/network-match.md) | Start a LAN Deathmatch after all participants are ready. Use six local and remote persons whose profile appearances differ from the built-in network default. Use a level with no locally usable named background, a recorded stable ordered eligible background list, and recorded replicated session, match, round, and level logical identities. Sustain an applicable network-budget breach until the degraded state appears while complete canonical updates continue. | Use six living players across connected participants during active play from the host view. Show the deterministic built-in fallback expected for the recorded identities and eligible list. | 1280x900 | One undivided arena must preserve the complete level, selected fallback background, six players, ranking, round progress, events, and player status. All six players must use the built-in default network player skin. The bottom status region must show `Host`, `LAN session`, `Connected`, `Session only scores`, `Optional scripts disabled`, and exact `Network connection degraded.` without overlap, clipping, truncation, disconnect, pause, reconnect, host-end, or background-fallback copy. The evidence packet must record the identity inputs, eligible list, named-background usability, and selected local filename outside the UI. | `docs/screenshots/NET-05/six-player-lan-degraded-1280x900.png` | PR #83 accepted at `e70a057819c97100b083c3cdaae5dc24566435cd`; SHA-256 `beed25818fc16387bdabea6533ff3bb4f74e8367fcf85128a6eb45d74df2be64`. Temporal, equal-client, reconnect, resynchronization, variation, and named-background evidence is recorded in the PR #83 packet section. `Conforms`. |
| <a id="ss-020"></a>`SS-020` | `NET-06` | [Completed final summary](../screens/wireframes/network-summary.md) | Complete a configured three-round network match. | Completed authoritative three-round result for all connected participants. | 1280x900 | Summary shows `Completed`, `Session only`, the final-round outcome as match outcome, the separately labeled last completed-round outcome, cumulative ranking without a champion treatment, no persistence, applicable actions, retained/departed rows, clearing at new-match start, and discard on session end. It does not show an interrupted result. | `docs/screenshots/NET-06/final-three-round-summary-1280x900.png` | PR #83 accepted at `e70a057819c97100b083c3cdaae5dc24566435cd`; SHA-256 `bbbd95719d079fc1d4fb62b3cf7e97ede165601ed81bc19fb4e1da7db2305dc2`. `Conforms`. |
...
- Fully conforming screenshot entries: 21.
...
- Coverage status: all 21 implemented entries conform.
