...
| <a id="ss-021"></a>`SS-021` | `NET-07` | [Guest reconnect](../screens/wireframes/network-reconnect.md) | Interrupt a guest connection during an active network match. | Reconnecting with 24 positive ceiling seconds remaining after an ambiguous retryable transport failure. | 1280x900 | Endpoint, unchanged reservation deadline, no-input state, active simulation, retryable status, and `Leave session` consequence are visible; the state does not claim host end or player removal. | `docs/screenshots/NET-07/reconnecting-24s-1280x900.png` | `Planned` for downstream issue #38; reconnect is not implemented. |
| <a id="ss-022"></a>`SS-022` | `NET-08` | [Connection failure](../screens/wireframes/network-failure.md) | Attempt to join an unavailable direct endpoint. | Initial `Host unreachable` with endpoint context, Retry, Edit setup, and Return to Network. | 1920x1080 | Initial admission uses fixed precedence and non-disclosing copy. Terminal reconnect variants disable Retry and expiry says only `Reconnect time expired. The session could not be restored.` | `docs/screenshots/NET-08/host-unreachable-1920x1080.png` | `Planned` for downstream issue #38; no current network UI exists. |
| <a id="ss-023"></a>`SS-023` | `NET-09` | [Host-ended/definitive-termination overlay](../screens/wireframes/network-host-loss.md) | Receive independently definitive session termination during an active network match. | Definitive termination over the last authoritative arena context. | 1280x900 | A blocking `SESSION TERMINATED` overlay is distinct from valid host-end copy, states no migration/resume or persistence, and cannot arise from silence or guest isolation alone. | `docs/screenshots/NET-09/session-terminated-1280x900.png` | `Planned` for downstream issue #38; host-ended/termination UI is not implemented. |
