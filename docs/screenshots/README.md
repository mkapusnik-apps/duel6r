...
PR #83 supplies the implemented issue #38 entries and refreshes `SS-001` and `SS-024` for the four-action footer.
The matrix contains 23 accepted entries. The five current network representatives retain their original capture hashes; corrected guest bounds and extracted summary evidence complete the assessment at approved source `6aebaf22d73a3d9409111714d3ebb7c2cf6a12ef`.
The earlier PR #83 acceptance at `e70a057819c97100b083c3cdaae5dc24566435cd` does not cover the corrected NET-04, NET-05, or NET-06 presentation.
...
The matrix contains exactly one representative entry for each of its 23 wireframes, including NET-04-R and NET-05-C.
The legacy owning specifications and wireframes include the integrated correction direction and links to the explicit variants.
Issue #34 defines presentation-independent replication semantics and stable identities and has exactly zero capturable graphical entries.
It must not implement `NET-04`, `NET-06`, or another graphical network screen.
Its result-semantics clarification adds no screen, wireframe, or representative screenshot entry.
The issue #38 entries must not be used as issue #34 evidence.
Issue #35 changes the representative target state for existing entry `SS-019` to the degraded-network variant of `NET-05`.
Issue #35 implements a production presentation handoff without a graphical consumer and does not change a rendered application screen.
Issue #35 adds no screenshot entry because `NET-05` keeps one stable wireframe.
`SS-019` keeps the degraded-network variant; its fresh capture resolves the former whole-player omission during Invisibility.
The approved first-release default-network-visual change applies to accepted `SS-016` through `SS-019` without changing their IDs or destination paths.
The same change applies to the retained arena in accepted `SS-021` and `SS-023` without adding a screenshot entry.

## PR #83 final issue #38 visual packet

- Branch and accepted source head: `feature/deliver-host-join-lobby-status-error-ux` at `e70a057819c97100b083c3cdaae5dc24566435cd`.
- Environment: Release, GL4, Lua enabled, Ubuntu 24.04 Docker, Mesa software rendering, dummy SDL audio, and isolated Xvfb at each matrix viewport.
- Executable SHA-256: `91247b9e4781e8ead2303e60291ca304695d410b967ddad6ba0d78501b4f9722`.
- External evidence manifest: `/tmp/opencode/duel6r-evidence-e70a057/evidence-manifest.md`.
- External evidence manifest SHA-256: `47f04c3b999a49c6159278b7a2b59da246737378f7568a4389c1fdbdc1d5149d`.
- Packet payload aggregate SHA-256: `bfb4a9bd1bc3053d01a7e9e6cb5bd03d2e4fac09fef9c7225453c1134ab094fa`.
- Canonical 13-file aggregate SHA-256: `12dbc9f44354f78e18aa2e0885d18aa0c0c8149f0643706a4f7b59a675ee9794`.
- Capture used project-local `127.0.0.1` communication and did not change host or container network infrastructure.
- `SS-019` records session `1782883788405837319`, match `144115188075855873`, round `216172782113783809`, logical level `levels/duel_16.json`, eligible backgrounds `000.tga` through `013.tga`, no usable named background, and selected fallback `001.tga`.
- Equal-client host and guest evidence SHA-256 values are `93be2173a8b70da970fedf296fbf741125cfa02c017a2d143c2ae73c18bc52d1` and `33c0dc77ed2604b4ec0f62a1bde9ca35c69283e43f847d1a23078d92ac1bf4e3`.
- Supported conditions resumed at `2026-09-14T10:43:49.377516Z`.
- The degraded indication remained visible through 3.122940468 seconds and first appeared cleared at 3.242767138 seconds.
- Named-background precedence evidence SHA-256 is `1d2534eaa0e6720d777756302a9d55acd1774aecbd8f0e3482d1e81ab05f8d2a`.
- Reconnect restoration and resynchronization evidence SHA-256 values are `1b836b0359653f50506730433011e1bd570143bceee23ca86db2491c3c090375` and `02ce65ad59082d9a2d841258b3287fd6d59baca0d34d46ee72d7c1cd1aa66c95`.
- The final assessment accepts all 13 canonical artifacts against their screen specifications, wireframes, and shared design rules.
...
Each new or replaced artifact must also include its SHA-256 and presentation profile.
Pending correction entries use source checkpoint `<developer-presentation-source-SHA>` until developer supplies the exact checkpoint.
No pending path is a captured artifact or conformance claim.
...
Fresh menu captures must use a 1920 by 1080 px client through a release display or an equivalent recorded capture configuration unless the matrix explicitly selects another supported viewport. SS-025 uses 1280 by 900 px to preserve the uninterrupted same-host SS-020 result.
...
- Historical evidence status: this artifact conformed to `UI-GAME-001`–`UI-GAME-004` and `AC-070`–`AC-071` before the current `UI-GAME-005`–`UI-GAME-007` update.
...
| <a id="ss-012"></a>`SS-012` | `OVER-03` | [Game over](../screens/wireframes/overlay-game-over.md) | Start a one-round local four-team Team deathmatch and finish its only round. | Use 15 ranked players. Put four players in Alpha, Bravo, and Charlie. Put three players in Delta. Use non-zero K, A, D, K/D, and PTS values. Use one winning team. Capture while the final summary and curtain are visible. | 1280x900 | The oversized final Team score panel, dark red curtain, and final round counter must appear over one undivided arena. The top-center counter must show final-round values, use the same visible 32 px character height as `---SCORE---`, and remain fully inside its measured opaque backing. The layout must use the complete available middle height before the score panel overlaps the counter backing. Any overlap must use only the minimum required vertical depth. No overlap may obscure, clip, scale down, or reduce the contrast of counter text or score content. Each team row must touch its nested player rows. Each of the three adjacent Team boundaries must match the 8 px band and centered 2 px rule used in non-final `OVER-02`. A horizontally centered blue notice at the bottom must show exact white text `End of Game`. The notice must keep its 16 px bottom inset, remain separate from the panel by at least 16 px, and must not obscure any score content. | `docs/screenshots/OVER-03/shared-game-over-1280x900.png` | The current oversized 15-player Team artifact conforms. The counter, backing overlap, complete score table, three Team separators, bottom notice, viewport containment, and contrast match the canonical requirements. `Conforms`. |
| <a id="ss-013"></a>`SS-013` | `CONS-01` | [Console over menu](../screens/wireframes/console-menu.md) | After issue #38 implements the Network footer, open the console over the populated local menu. | Use the planned four-action footer, recent startup output, and an empty input line. | 1920x1080 | The console spans the client width while the visible scaled menu below includes equal-width `Play (F1)`, `Network (F2)`, `Clear (F3)`, and `Quit (ESC)` actions. | `docs/screenshots/CONS-01/network-entry-open-1920x1080.png` | PR #83 accepted at `e70a057819c97100b083c3cdaae5dc24566435cd`; SHA-256 `2d40fcbb8d57813fe17f33ced32598bcf1be8925a77078bcf93cdc07ee5a7ba8`. `Conforms`. |
...
| <a id="ss-015"></a>`SS-015` | `NET-01` | [Network entry](../screens/wireframes/network-entry.md) | From `MENU-01`, activate `Network (F2)`. | Default target state with Host focused and direct same-machine/LAN scope visible. | 1920x1080 | The scaled retro canvas shows Host, Join, Back, supported platforms, direct endpoint scope, host-alone lobby 1–15, match 2–15, and no unsupported affordances. | `docs/screenshots/NET-01/default-1920x1080.png` | PR #83 accepted at `e70a057819c97100b083c3cdaae5dc24566435cd`; SHA-256 `16d65732b2562a68c7ddb1f76f9e2ccf0012a06fddf877faea2b22b2ef1d8a26`. `Conforms`. |
| <a id="ss-016"></a>`SS-016` | `NET-02` | [Host setup](../screens/wireframes/network-host-setup.md) | Choose Host from `NET-01`, keep Port as the initial focus, move next to `Listening interface`, and explicitly select one enumerated eligible assigned private RFC1918 IPv4 address without changing network infrastructure. | Editable host setup with the selected private address, a valid port, and two configured host local-player slots. Give the two people different available local profile appearances. | 1920x1080 | Port remains above the focused `Listening interface` selector. The selector shows the complete selected IPv4 literal and `Private LAN`. The split Persons and Local Players panels keep their documented allocation. Two-slot count, pre-admission Add and Remove, person selection, control assignment, capacity, Start session, Back, and validation remain visible without overlap or displacement. No profile UI, profile-difference warning, discovery, Internet, NAT, port-forwarding, or infrastructure-mutation control appears. The loopback-default and expanded eligible-address-list states remain required variants. Start revalidates the selected address and finalizes the displayed slot count. Pending variants lock setup and expose the 10-second boundary and Cancel. | `docs/screenshots/NET-02/host-setup-1920x1080.png` | PR #83 accepted at `e70a057819c97100b083c3cdaae5dc24566435cd`; SHA-256 `3e4b93846b936e58e72ee1b5cd35f154495abd9b809819e5afde58387c04c260`. `Conforms`. |
| <a id="ss-017"></a>`SS-017` | `NET-03` | [Join connecting](../screens/wireframes/network-join.md) | Start a project-local host process bound to `127.0.0.1:26660` that accepts the loopback connection and withholds a complete terminal admission result until after capture. Do not change host, Docker, firewall, route, NAT, or port-forwarding configuration. Choose Join, enter `127.0.0.1:26660`, configure two local-player slots with different available local profile appearances, and activate Connect. | Connecting to `127.0.0.1:26660` before the original 10-second deadline, with the finalized two-slot count and Cancel focused. | 1920x1080 | The loopback endpoint, finalized person selection, and control assignment remain visible with truthful `Connecting to 127.0.0.1:26660…` copy and the 10-second total boundary. Slot count and order are locked. No active Add, Remove, or Transfer action appears. No profile or cosmetic-compatibility control or warning appears. Admission is not claimed before exact confirmation, calibration, and a matching complete initial snapshot. Focused behavioral revalidation must show that Cancel restores the retained endpoint and two slots. A separate run that withholds every complete terminal result through the deadline must show `Connection timed out.` This same-machine capture complements and does not replace the explicit private-LAN coverage in `SS-016`. | `docs/screenshots/NET-03/connecting-two-local-players-1920x1080.png` | PR #83 accepted at `e70a057819c97100b083c3cdaae5dc24566435cd`; SHA-256 `f8accf3d2284914c510061b2831e6fc4d4e7c3dffc84b50c988e05af1dc5ab7e`. `Conforms`. |
| <a id="ss-018"></a>`SS-018` | `NET-04` | [Lobby](../screens/wireframes/network-lobby.md) | Host with two guests and two slots each. Select Team deathmatch, four teams, Friendly Fire on; switch to Deathmatch, then Predator; make another permitted setting edit. | Host lobby in Predator after readiness clearing; host and Guest 2 readied again, Guest 3 remains the named Start blocker. | 1920x1080 | Current membership, separate role/connection/readiness/ownership, person/control edits and host settings remain contained. Non-Team effective settings show no teams or Friendly Fire. No hidden Team-control focus or pointer targets remain. Behavioral follow-up must restore both preferences on return to Team deathmatch under NET-SET-AC-001. | `docs/screenshots/NET-04/host-waiting-one-unready-1920x1080.png` | `Conforms` for the representative visual scope. Capture checkpoint `24d244bee7edd5523c5791297cabe1b00c01fd7e`; supplied SHA-256 `d9aa586e3c2d2d95ffdea39bfd5076811013f8e32c33dc5635c1f862eb75e1a1`. Readable role/readiness/ownership, six-player roster, named blocker and contained footer; Team controls absent. The inactive fixed-level value shows a clipped Unknown-level placeholder under Random, not the former stale selected level. This is a non-blocking copy observation; no added UX scope is required. |
| <a id="ss-019"></a>`SS-019` | `NET-05` | [Network match](../screens/wireframes/network-match.md) | Start the existing six-player LAN Deathmatch with default visuals and a recorded deterministic background fallback. Reach an authoritative invisibility bonus while a weapon is held. Sustain degraded status while complete updates continue. Record the affected player, bonus state and weapon outside the UI. | Host active arena, six living players, one body and held weapon at the Invisibility opacity, no confirmation. | 1280x900 | Preserve undivided arena, ranking, progress, events and player status. Keep Host, LAN session, Connected, Session only scores, Optional scripts disabled and Network connection degraded. readable. Under BON-013, Invisibility must render both body and held weapon at 20% opacity; neither complete omission nor an opaque weapon conforms. Separate temporal evidence must show Predator's 10% body opacity with a visible weapon when invisibility is absent. | `docs/screenshots/NET-05/six-player-lan-degraded-1280x900.png` | `Conforms` for the representative visual scope. Capture checkpoint `24d244bee7edd5523c5791297cabe1b00c01fd7e`; supplied SHA-256 `8e91fa6a693ff328271289d3198cf3d55c7f4812d8e98efe06abd2c05c110154`. Translucent presentation replaces the omitted player; packet state identifies Ada/player 2, alive, visible=true, alpha 51, Invisibility and held machine gun. Arena and status remain contained. Capture is degraded recovery after the owned-server stall, not uninterrupted simulation through it. |
| <a id="ss-020"></a>`SS-020` | `NET-06` | [Completed final summary](../screens/wireframes/network-summary.md) | Start the host at 1280x900. Complete three Predator rounds with 15 players across three participants. In the final round, kill the Predator while all 14 Marines survive. Use supported maximum-length 64 UTF-8-byte winner names. A winning guest leaves in final summary. Scroll to expose a departed winner's complete name and identity. Continue directly to SS-025 on this same host without restarting or resizing. | Host completed summary with retained multiwinner outcomes and focused result scrolling. | 1280x900 | Keep panel bounded with at least 16 px client inset, fixed heading/outcome labels and footer. Keep match outcome, last-round outcome and ranking separate. Long headers must not overflow: use a bounded winner count and See outcome rows cue when needed. Scrollable outcome rows must expose every winner's full name, identity, scope and Departed state. Clip only the viewport, not stored text; preserve UTF-8 boundaries. Keep headings and row/column position visible. Do not shrink text or label the ranking leader champion. | `docs/screenshots/NET-06/final-three-round-summary-1280x900.png` | `Conforms`. Capture checkpoint `24d244bee7edd5523c5791297cabe1b00c01fd7e`; supplied SHA-256 `6fa429de1cf74a74cad867b4817bdcdd06b08211476072bc2bb8fc0e1a8c1518`. Separate bounded outcomes, complete G1 name/player 8/Departed, scroll position and footer are readable. Extracted 1280x720, 850x700 and horizontal-scroll evidence conforms: row endings are accessible and remain inside the result viewport while headings and actions stay fixed. Hash reused at final assessment; summary presentation is unchanged by the guest-status fix. |
| <a id="ss-021"></a>`SS-021` | `NET-07` | [Guest reconnect](../screens/wireframes/network-reconnect.md) | Interrupt a two-slot guest connection during an active network match that uses persons with available profile appearances different from the built-in network default. | Reconnecting with the same two reserved owned slots and 24 positive ceiling seconds remaining after host crash or another ambiguous transport failure. | 1280x900 | Endpoint, unchanged reservation deadline, `Your player slots are reserved`, no-input state, active simulation, retryable status, and `Leave session` consequence are visible. The retained arena keeps the same reserved players and default network visuals. No add, remove, replacement, reorder, transfer, host-end, or player-removal claim appears. | `docs/screenshots/NET-07/reconnecting-24s-1280x900.png` | PR #83 accepted at `e70a057819c97100b083c3cdaae5dc24566435cd`; SHA-256 `5821048b6e768f8f5b8f50199c7e77de9ff38fb6c052536210c49438585a82b8`. `Conforms`. |
| <a id="ss-022"></a>`SS-022` | `NET-08` | [Connection failure](../screens/wireframes/network-failure.md) | Attempt to join an unavailable direct endpoint. | Initial `Host unreachable.` with endpoint context, Retry, Edit setup, and Return to Network. | 1920x1080 | Initial admission uses fixed precedence; terminal reconnect disables Retry; expiry uses truthful copy; host-local service failure uses `Hosted session stopped unexpectedly.` only for the host. | `docs/screenshots/NET-08/host-unreachable-1920x1080.png` | PR #83 accepted at `e70a057819c97100b083c3cdaae5dc24566435cd`; SHA-256 `251d7c91c5d5ac1464a174edbc8545ba09f3ea0123ea7716350a3c707f779657`. `Conforms`. |
| <a id="ss-023"></a>`SS-023` | `NET-09` | [Host-ended session overlay](../screens/wireframes/network-host-ended.md) | Receive a valid intentional host End session notice during an active network match that uses persons with available profile appearances different from the built-in network default. | Host-ended outcome over the last authoritative arena context. | 1280x900 | A blocking `HOST ENDED SESSION` overlay states no migration/resume or persistence and can arise only from an End notice accepted through the current established session. The retained arena uses the default network visuals and does not switch to selected-profile appearances. | `docs/screenshots/NET-09/host-ended-1280x900.png` | PR #83 accepted at `e70a057819c97100b083c3cdaae5dc24566435cd`; SHA-256 `08dae876649ac62137d7b3d6939ed1bfad00c2265e373fb1001b1fcdee93329e`. `Conforms`. |
| <a id="ss-024"></a>`SS-024` | `MENU-01` | [Teams Game Settings](../screens/wireframes/menu-main.md#menu-01-b--teams-state) | Open the main menu, select six players, and select `Teams`. | Use eight saved persons, six selected players, two teams, Friendly Fire off, Rounds `0`, visible `Equalize` and `Shuffle`, and Cora selected. | 1920x1080 | Persons must show `Remove` at bottom-left and `>>` at bottom-right. Players must show `<<` at bottom-left and `Detect All` at bottom-right. `Equalize` and `Shuffle` must form a centered group in the available span between the Players edge controls. All six buttons must use one common height with visible caption padding. The action rows must align and remain contained without overlap. | `docs/screenshots/MENU-01/teams-person-action-aligned-1920x1080.png` | PR #83 accepted at `e70a057819c97100b083c3cdaae5dc24566435cd`; SHA-256 `48b9147105c248200f74d696260329b92b344d831966e65b373a12e80e1441e7`. `Conforms`. |

## Explicit structural variants

These entries supplement the unchanged screen IDs without moving the legacy owning documents. Each SVG owns only the specified additional structural variant. Functional behavior remains defined by the linked product contract. Existing representative artifacts are replaced at their existing destinations; genuinely new variant captures use `docs/design/screenshots/`.

| Entry | Screen / wireframe | Structure and contract | Setup route and representative state | Viewport / profile | Expected presentation | Destination | Conformance |
|---|---|---|---|---|---|---|---|
| `SS-025` | `NET-04` / `NET-04-R` | [SVG](../design/wireframes/NET-04/NET-04-R.svg); [lobby contract](../screens/network-lobby.md); NET-RES-001/002/AC-001 | Continue SS-020 uninterrupted on the same host at the same 1280x900 startup viewport after a five-player winning guest leaves. Host activates Return to lobby without restarting or resizing. Capture two current participants, ten current players, readiness cleared, retained final-round winners and a departed winner row. Focus result scrolling and expose a complete maximum-length name and player identity. | 1280x900 desktop; scaled 850x700 logical menu | Fixed header and footer; current membership/settings above approximately 215 logical px of retained results. Keep completed outcome labels separate from cumulative ranking. Historical winners remain readable and Departed; removed identities do not appear in membership. Full names/identities and every available outcome remain reachable through existing horizontal/vertical scrolling. | `docs/design/screenshots/NET-04/NET-04-R.png` | `Conforms`. Capture checkpoint `24d244bee7edd5523c5791297cabe1b00c01fd7e`; supplied SHA-256 `dba2ee0762f48d9061fae5e375dd41a43bbaff9b5c2a1e9fabb40cabbba4ea77`. Two participants/ten players and cleared readiness remain separate from fourteen retained winners; complete G1 identity and Departed are readable. Extracted horizontal evidence conforms. Four actual guest captures at `6aebaf22d73a3d9409111714d3ebb7c2cf6a12ef` resolve the status collision and preserve Person/Control focus, results and footer at 850x700 and 1280x720. Reuse this unchanged same-host representative with the corrected guest supplements. |
| `SS-026` | `NET-05` / `NET-05-C` | [SVG](../design/wireframes/NET-05/NET-05-C.svg); [match contract](../screens/network-match.md); NET-AC-016 | During a connected active match, a guest explicitly opens Leave session using Escape or controller Back. Capture before acceptance with Leave session focused in the existing Confirm-then-Cancel order. | 1280x720 desktop | One arena behind a centered bounded panel; exact match Leave consequence wraps inside it; Leave session and Cancel remain distinct, fully visible and focusable. Keep at least 16 px client inset and 8 px between controls. The host End variant uses the same structure and existing exact copy. Opening input must not also confirm. | `docs/design/screenshots/NET-05/NET-05-C.png` | `Conforms` for the representative visual scope. Capture checkpoint `24d244bee7edd5523c5791297cabe1b00c01fd7e`; supplied SHA-256 `29b60a28f470556f15adcba8a2165df7f529b613cb17dcfa74ed637d27d56744`. Centered contained panel, complete consequence copy, separated actions and non-color focus outline match NET-05-C. Existing Confirm-then-Cancel order is preserved. Behavioral QA, not this still image, establishes input isolation and action outcomes. |

### Supported presentation and non-static checks

The five current representative images were read and assessed at capture checkpoint `24d244bee7edd5523c5791297cabe1b00c01fd7e`. The packet identifies byte-unchanged production source `f7730db176f48bfd6436b634b88a2d18310bd221`, Release GL4, Lua ON, Ubuntu 24.04 Docker, Xvfb 24-bit startup viewports, software rendering and dummy SDL audio. Developer reports complete unedited client captures from normal flat runtime directories without the former nested-resources workaround. The supplied artifact hashes are recorded in the matrix; they were not independently recomputed by UX. Cross-checkpoint behavioral QA approval is supplied by team and is not inferred from still images.

Runtime artifact SHA-256 from the handoff: `fb4aa847ec02f893095c1236aa1e439ca9258c15901e07200ea39f64e11c9496`. The user separately supplies capture-packet tar SHA-256 `2adad3c2bb46273ece67913f62aaf2fe96a7451cc003f832367939a58bf2ce64`; it is not the runtime artifact hash.

SS-019 uses session `376367718770980310`, match `144115188075855873`, round `216172782113783809`, level `levels/duel_16.json`, mirrored=false and near-capture tick 13299. The eligible background list is `000.tga` through `013.tga`; the packet records no named background and selected fallback `006.tga`. The near-capture state-file SHA-256 is `0ade896d7752643055aa430a7fa1b9a1e2dcf4c7b445a0bda8a4c27c5c8fae07`. The actual pickup and resumed-update recovery frame supersede the former omitted-player defect evidence.

SS-020 and SS-025 retain session `1916732464373467639`, match `144115188075855873`, round `216172782113783811`, tick 17027 and completed round 3. The packet records fourteen final winners and five departed G player identities 8–12. Normal gameplay produced the result. Maximum-length names were loaded through normal personal save records, not injected network state. The same host continued from final summary to the ten-player lobby at 1280x900 without resize or restart. State-file SHA-256 values are `c93bfe6b2b491e324ade29ecc917b28d0d7c1607179006f0219215655db672f5` and `c6d18db2148037dd181ceadc509c2092d71bafbe8a538d537efc50b2e7f9fc2f`, respectively.

### Guest retained-lobby final assessment

The four corrected guest captures conform at approved source `6aebaf22d73a3d9409111714d3ebb7c2cf6a12ef`. The raw lowercase `lobby` status remains present in canonical metadata but is absent from the rendered status area. The read-only explanation and script policy are separate and readable. Person and Control have distinct contained focus outlines at 850x700 and 1280x720. Result headings, rows, scroll feedback, Ready and Leave remain visible without overprint. The previous guest-status blocker is resolved.

The recapture uses a real three-player, one-round Predator result with two winners; it supplements rather than replaces the fourteen-winner canonical pair. Session `6627074416700005532`, match `144115188075855873`, round `216172782113783809`, completed round 1, tick 20402 and winner identities 2 and 4 identify the actual retained state. The source/QA approval covers meaningful UTF-8 status bounds. No naturally emitted long non-ASCII status was available, and no synthetic status was supplied. These images establish neutral-status and focus presentation, not a captured long-status state. No additional capture of an unreachable state is required.

Environment: Ubuntu 24.04 Docker, Release GL4, Lua ON, Xvfb 24-bit startup viewports, software rendering and dummy SDL audio; normal flat runtimes, real SDL input and natural drowning/winner progression. Artifact SHA-256: `f1b27d9e6e29d859f580b8df1347114135a67d6ec6cf5600c812c6d5cae99d4a`. Application SHA-256: `f0e2a0c1bc70a201e9f8d8c5371cd2e30359c7e85c5942c28405b1694cf9bd0f`. Read-only state SHA-256: `a9b5ef6fc3557629cf874cb66878dd0b0495135c86cd7876ca3053d6ccb0c27c`.

Retain the following four accepted supplemental artifacts under `docs/design/screenshots/NET-04/`. Developer must copy the exact bytes from `/tmp/opencode/pr83-guest-status-6aebaf2/` using the same basenames and verify the supplied hashes. The files were assessed at those external source paths; the canonical copies are an integration handoff, not already-created files. These are bounds/focus evidence for NET-04-R, not new matrix entries.

| Destination basename | Viewport / focus | SHA-256 | Assessment |
|---|---|---|---|
| `retained-person-focus-850x700.png` | 850x700 / Person | `7e7d9f75d877b85ebc416aeed4a32ae7956f9bca9ed73176c5458a33dc69eed2` | Conforms; phase label omitted, read-only copy separate, Person focus contained. |
| `retained-control-focus-850x700.png` | 850x700 / Control | `309b03cf4f2e6e13a72e0a267a01e425bd5090255a0d880467db5aa3dd665db6` | Conforms; distinct Control focus, unchanged readable results and footer. |
| `retained-person-focus-1280x720.png` | 1280x720 / Person | `1ae5fc1da532839ddcbd6ea156e9443e1e423758def4cb425c681d3df9114251` | Conforms; scaled layout and Person focus remain contained. |
| `retained-control-focus-1280x720.png` | 1280x720 / Control | `fb0e2446343e6b6b84edfdfb99ff733393b00819db5f739c35c369b07d772439` | Conforms; scaled Control focus and status separation remain readable. |

### Assessed unchanged supplemental evidence

The following eight images were read from `/tmp/opencode/pr83-capture-24d-images/` with their original `24d244bee7edd5523c5791297cabe1b00c01fd7e` capture provenance. They remain valid because the guest-status correction does not change their presentation. They are supplemental evidence, not additional representative entries.

| File | SHA-256 | Assessment |
|---|---|---|
| `lobby-1280x720.png` | `c4108cad49c27745bcef28b0d69af20dc663defc9bb2b55a7b7ecc85c1664b5d` | Conforms; six-player membership, settings, permission copy and footer fit. |
| `confirmation-1280x900.png` | `b2cc3b4ce04c49877fe1f6074bca13dfc1c72880549cebed38325ec04413aebe` | Conforms; complete consequence and distinct buttons remain centered and contained. |
| `summary-1280x720.png` | `7c2b050ed9af37f840d9fdfb33e3c1f6fa365c020f9eacc5e7a7db6cf8f62fd9` | Conforms; fixed outcome labels, result rows, position feedback and guest action remain readable. |
| `summary-1280x720-horizontal.png` | `35982e11e54fcbcda9b0024c970c0c337ef7b241be0423175e8f4cfc1ce77144` | Conforms; full name/identity/state endings become visible and headings track horizontal position. |
| `summary-850x700.png` | `0415331a0d9c0f9eb50fe923ee07bd591d231471c3b65156303e86cd8026bbf7` | Conforms; compatibility-floor outcome headings, rows, scroll controls and Leave fit without overlap. |
| `summary-horizontal-end.png` | `6ea8c6a5898863347fe134cd7a5b673bfc5737594709450347af4a69d0269227` | Conforms; full departed names/identities/state endings are accessible with fixed outcomes and actions. |
| `retained-horizontal-end.png` | `fc5e4c9c48630a46797b81eed20ded16ff04d16f2ba5c7febdde5a461f998c92` | Conforms; historical departed identity remains accessible separately from current membership. |
| `preferences.png` | `e54edf134fe76164ac3ce1dbaea33bd96d067f6989104ded0f8fa5002c560a79` | Conforms for the depicted Team state; four teams, Friendly Fire on and visible focus are contained. Temporal preference restoration remains established by approved behavioral evidence. |

Only the older guest-defect supplements `retained-850x700.png`, `retained-1280x720.png`, `retained-person-focus-850x700.png` and `retained-control-focus-850x700.png` from the 24d packet are superseded for current guest-status conformance. They are not evidence for the fixed state. No other prior artifact is invalidated by this localized correction.

### Final reuse and limits

- Preserve the original hashes and capture provenance of SS-018, SS-019, SS-020, SS-025 and SS-026.
- The unchanged same-host SS-020/SS-025 pair remains the fourteen-winner/five-departed representative at 1280x900.
- A commit that copies the accepted images byte-for-byte and integrates this exact manifest without changing presentation source does not require another UX assessment.
- Reassess only changed presentation source, image bytes or UX-owned requirements; no further capture is required for the accepted scope.
- Full-list navigation, input isolation, timings, rate limits, UTF-8 bounds and lifecycle outcomes use the supplied approved source/QA evidence where a still image cannot establish behavior.
- This visual acceptance does not establish parent network-release readiness or waive non-visual hosted gates.

- Developer must supply each required replacement through a reachable production state and record the source checkpoint, state/setup, environment, viewport/profile, path and SHA-256; pending supplemental assessment alone does not require recapturing an unchanged representative.
- No screenshot may substitute a synthetic painted state for an unreachable outcome.
- Menu allocation must remain usable at the 850x700 compatibility floor and 1280x720 evaluation minimum.
- Summary and confirmation containment must remain usable at 1280x720 and 1280x900.
- These sizes are checks of the same desktop presentation, not additional matrix entries or mobile layouts.
- NET-06 must expose each available outcome's full winner list before the existing settings, round-result and cumulative-result sections.
- An overlong NET-04 or NET-06 outcome header must keep a bounded winner count and a visible See outcome rows cue; its scrollable rows must retain complete names and identities.
- Outcome rows must identify match or round scope; ranking order must not imply a champion.
- Result headings and footer actions must stay fixed while rows scroll; horizontal headings must track their values.
- Existing keyboard/controller/pointer scrolling must reach every winner and the end of each maximum-length name without triggering an unrelated action.
- Verify Leave and expiry separately, then return directly to the lobby without restoring departed membership or reevaluating completed winners.
- Verify Tab press/release/press toggles the pre-winner scoreboard and does not dismiss a round result after an outcome exists.
- Verify active-play Return/Space cannot activate invisible End/Leave targets; retain activation of explicitly visible focused controls.
- Verify opening input, repeats and held input cannot accept a new confirmation or newly displayed round action.
- Preserve the existing confirmation action order and focus; this change does not introduce initial Cancel focus.
- Verify host End from lobby, match and summary completes without freezing the interface.
- Verify first-second living-player input, frozen-phase rejection, host-owned input throughput and existing advancement permissions with behavioral evidence.
- Verify reconnect Leave/restore race ordering behaviorally. NET-07 keeps its existing wireframe and screenshot; do not recapture it solely for this race correction.
- Verify finding #9 against the shared non-input guest action budget: 30 actions per second, burst 60, across Owned-person, Ready and other applicable non-input guest actions. This is not the aggregate player-input budget. No new rate UI or changed limits are approved.

## Coverage

- Required wireframes: 23, including two integrated explicit structural variants.
- Required representative screenshot entries: 23.
- Fully conforming screenshot entries: 23 (18 unchanged plus the five accepted network representatives and their required supplemental assessment).
- Known blocking visual defects in the assessed scope: 0.
- Planned screenshot entries: 0.
- Pending final-assessment or correction entries: 0.
- Screenshots per wireframe: exactly one.
- Retired screenshot matrix entries: `SS-004`, `SS-005`, and `SS-006`.
- Coverage status: satisfied at approved source `6aebaf22d73a3d9409111714d3ebb7c2cf6a12ef`, using the unchanged five canonical 24d images, eight unchanged assessed supplements and four corrected guest bounds/focus images. Exact-byte retention of the four corrected supplements remains developer's integration task, not a request for new capture.
