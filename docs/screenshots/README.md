...
| <a id="ss-017"></a>`SS-017` | `NET-03` | [Join connecting](../screens/wireframes/network-join.md) | Choose Join, enter `192.168.1.24:27015`, configure two local-player slots with different available local profile appearances, and activate Connect. | Connecting to `192.168.1.24:27015` with the finalized two-slot count and Cancel available. | 1920x1080 | Endpoint, finalized person selection, and control assignment remain visible with truthful Connecting copy and the 10-second total boundary. Slot count and order are locked, and no active Add, Remove, or Transfer action appears. No profile or cosmetic-compatibility control or warning appears. Cancel retains setup. Admission is not claimed before exact confirmation, calibration, and a matching complete initial snapshot. | `docs/screenshots/NET-03/connecting-two-local-players-1920x1080.png` | `Planned` for downstream issue #38; no current network UI exists. |
