...
| <a id="ss-020"></a>`SS-020` | `NET-06` | [Final summary](../screens/wireframes/network-summary.md) | Complete a configured three-round network match. | Final authoritative three-round result for all connected participants. | 1280x900 | Summary shows totals, `Session only`, no persistence, applicable actions, and supports retained/departed/replaced/discarded result variants. | `docs/screenshots/NET-06/final-three-round-summary-1280x900.png` | `Planned` for downstream issue #38; no current network UI exists. |
| <a id="ss-021"></a>`SS-021` | `NET-07` | [Guest reconnect](../screens/wireframes/network-reconnect.md) | Interrupt a guest connection during an active network match. | Reconnecting with 24 positive ceiling seconds remaining and reserved players in the continuing match. | 1280x900 | Endpoint, fixed reservation deadline, no-input state, active simulation copy, and `Leave session` with removal/no-reconnect consequence are visible. | `docs/screenshots/NET-07/reconnecting-24s-1280x900.png` | `Planned` for downstream issue #38; reconnect is not implemented. |
| <a id="ss-022"></a>`SS-022` | `NET-08` | [Connection failure](../screens/wireframes/network-failure.md) | Attempt to join an unavailable direct endpoint. | `Host unreachable` with endpoint context, Retry, Edit setup, and Return to Network. | 1920x1080 | The specific reason precedes generic timeout; endpoint/setup remain available for exact Retry, Edit setup, and Return destinations. | `docs/screenshots/NET-08/host-unreachable-1920x1080.png` | `Planned` for downstream issue #38; no current network UI exists. |
| <a id="ss-023"></a>`SS-023` | `NET-09` | [Host-ended/loss overlay](../screens/wireframes/network-host-loss.md) | Lose the authoritative host during an active network match. | Host lost over the last authoritative arena context. | 1280x900 | A blocking overlay distinguishes unexpected host loss from confirmed host end, states no migration/resume or persistence, and offers Return to Network without imposing a fixed menu canvas. | `docs/screenshots/NET-09/host-lost-1280x900.png` | `Planned` for downstream issue #38; host-ended/loss UI is not implemented. |
