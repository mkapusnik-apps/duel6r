...
The matrix contains 18 unchanged accepted entries and five pending entries.
...
The existing screen inventory still needs links to these two explicit variants; its owning directory is not writable by this UX session.
...
`SS-019` keeps the degraded-network variant and requires replacement evidence for authoritative invisibility of the body and held weapon.
...
| <a id="ss-018"></a>`SS-018` | `NET-04` | [Lobby](../screens/wireframes/network-lobby.md) | Host with two guests and two slots each. Select Team deathmatch, four teams, Friendly Fire on; switch to Deathmatch, then Predator; make another permitted setting edit. | Host lobby in Predator with readiness cleared and one named Start blocker. | 1920x1080 | Current membership, separate role/connection/readiness/ownership, person/control edits and host settings remain contained. Non-Team effective settings show no teams or Friendly Fire. No hidden Team-control focus or pointer targets remain. Behavioral follow-up must restore both preferences on return to Team deathmatch under NET-SET-AC-001. | `docs/screenshots/NET-04/host-waiting-one-unready-1920x1080.png` | `Pending replacement`; source `<developer-presentation-source-SHA>`; artifact SHA-256 pending. Earlier artifact does not establish corrected conformance. |
| <a id="ss-019"></a>`SS-019` | `NET-05` | [Network match](../screens/wireframes/network-match.md) | Start the existing six-player LAN Deathmatch with default visuals and a recorded deterministic background fallback. Reach an authoritative invisibility bonus while a weapon is held. Sustain degraded status while complete updates continue. Record the affected player, bonus state and weapon outside the UI. | Host active arena, six living players, one invisible body and held weapon, no confirmation. | 1280x900 | Preserve undivided arena, ranking, progress, events and player status. Keep Host, LAN session, Connected, Session only scores, Optional scripts disabled and Network connection degraded. readable. Invisibility must affect body and weapon; no opaque weapon may disclose the invisible player. Separate temporal evidence must show Predator's 10% body opacity with a visible weapon when invisibility is absent. | `docs/screenshots/NET-05/six-player-lan-degraded-1280x900.png` | `Pending replacement`; source `<developer-presentation-source-SHA>`; artifact SHA-256 pending. Keep existing background reproduction metadata requirements. |
| <a id="ss-020"></a>`SS-020` | `NET-06` | [Completed final summary](../screens/wireframes/network-summary.md) | Complete three Predator rounds with 15 players across three participants. In the final round, kill the Predator while all 14 Marines survive. Use supported maximum-length 64 UTF-8-byte winner names. A winning guest leaves in final summary. Scroll to expose a departed winner's complete name and identity. | Host completed summary with retained multiwinner outcomes and focused result scrolling. | 1280x900 | Keep panel bounded with at least 16 px client inset, fixed heading/outcome labels and footer. Keep match outcome, last-round outcome and ranking separate. Long headers must not overflow: use a bounded winner count and See outcome rows cue when needed. Scrollable outcome rows must expose every winner's full name, identity, scope and Departed state. Clip only the viewport, not stored text; preserve UTF-8 boundaries. Keep headings and row/column position visible. Do not shrink text or label the ranking leader champion. | `docs/screenshots/NET-06/final-three-round-summary-1280x900.png` | `Pending replacement`; source `<developer-presentation-source-SHA>`; artifact SHA-256 pending. NET-RES-001/002 and NET-RES-AC-001. Legacy owning specification and diagram update remain blocked by tool permissions. |
...
- Required wireframes: 23, including two explicit structural variants awaiting inventory cross-links.
- Required representative screenshot entries: 23.
- Fully conforming screenshot entries: 18 unchanged entries.
- Non-conforming screenshots: 0.
- Planned screenshot entries: 0.
- Pending screenshot entries: 5.
- Screenshots per wireframe: exactly one.
- Retired screenshot matrix entries: `SS-004`, `SS-005`, and `SS-006`.
- Coverage status: blocked for NET-04, NET-04-R, NET-05, NET-05-C and NET-06 pending evidence and the blocked legacy specification updates.
