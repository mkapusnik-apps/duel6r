...
`CONS-01` remains valid from the earlier implementation source because the console fully covers the changed Players header.
...
|---|---|---|---|---|---:|---|---|---|
| <a id="ss-001"></a>`SS-001` | `MENU-01` | [Main menu](../screens/wireframes/menu-main.md) | Launch the local application to the setup menu. | Use four saved people, two selected players, Deathmatch, persistent score data, Assistance on, Quick Liquid on, and Rounds `0`. | 1706x938 | The black matte must surround the centered 850x700 grey canvas. The animated banner and current runtime version must appear above the four ordered panels. Each selected player must align with a control assignment. The full statistics table and action-first footer captions must be visible. | `docs/screenshots/MENU-01/default-1706x938.png` | Player-count replacement provenance above. `PLAYERS 2` is visible, `CONTROLLER` remains aligned, and the complete menu presentation conforms. `Conforms`. |
| <a id="ss-002"></a>`SS-002` | `MENU-02` | [Menu message](../screens/wireframes/menu-message.md) | Select Clear or press F3 from the populated menu. | Use the `SS-001` setup and show `Really delete? (Y/N)` over the unchanged menu. | 1706x938 | The centered strip must show the exact confirmation with the documented surface, text, and frame. The black matte and approved four-panel menu must remain visible behind it. | `docs/screenshots/MENU-02/confirmation-1706x938.png` | Player-count replacement provenance above. `PLAYERS 2` remains visible behind the conforming confirmation strip. `Conforms`. |
