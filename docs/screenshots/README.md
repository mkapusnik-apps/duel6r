...
`SS-012` is current and conforms to the `OVER-03` oversized-panel wireframe and `UI-GAME-001`–`UI-GAME-007` and `AC-070`–`AC-072`.
...
The planned issue #38 entries must not be used as issue #34 evidence.
...
`SS-019` remains `Planned` because issue #38 owns graphical consumption and capture of the `NET-05` target.
The approved first-release default-network-visual change updates planned `SS-016` through `SS-019` without changing their IDs or destination paths.
The same change updates the retained arena expectations for planned `SS-021` and `SS-023` without adding a screenshot entry.
No existing implementation artifact becomes stale because all six affected entries remain `Planned` and have no accepted artifact.
