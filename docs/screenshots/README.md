...
| <a id="ss-016"></a>`SS-016` | `NET-02` | [Host setup](../screens/wireframes/network-host-setup.md) | Choose Host from `NET-01`. | Valid port and two configured host local-player slots in editable host setup. Give the two people different available local profile appearances. | 1920x1080 | Host setup shows retained port, two-slot count, pre-admission Add and Remove, person selection, control assignment, capacity, Start session, Back, and validation. It shows no profile selector, profile column, profile value, profile-editing action, or profile-difference warning. Start session finalizes the displayed count for its attempt. Pending variants lock slot count and order and expose the 10-second boundary and Cancel. | `docs/screenshots/NET-02/host-setup-1920x1080.png` | `Planned` for downstream issue #38; no current network UI exists. |
