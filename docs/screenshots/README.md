...
At the consolidated Teams approval, the two entries used specification `ea5d918cf4d9f9402addfbc341759cd428ce5441` on `docs/consolidate-team-mode` as their historical requirements baseline.
...
Issue #34 defines presentation-independent replication semantics and stable identities.
Issue #34 has exactly zero capturable graphical entries because it must not implement `NET-04`, `NET-06`, or another graphical network screen.
The issue #34 clarification changes target result semantics but adds no screen, wireframe, or representative screenshot entry.
The 21-entry wireframe-based screenshot matrix remains unchanged with exactly one representative entry per wireframe.
The existing planned issue #38 entries must not be used as issue #34 evidence.
...
The 2026-09-01 consolidated-person-list specification affects both `MENU-01` wireframes.
The fixed product baseline is `docs/features.md` at source commit `fed0daa3939dd517f29a7434b99b1009b30be848`.
The consolidated-person-list requirements use `SET-048`–`SET-072` and `AC-053`–`AC-062`.
The Equalize and Shuffle requirements use `SET-017`–`SET-019`, `SET-073`–`SET-077`, `AC-011`, and `AC-063`–`AC-064`.
The change invalidates the conforming PR #54 artifacts for `SS-001` and `SS-024` because both artifacts show separate `ELO SCOREBOARD` and `PERSONS` lists.
The historical [`MENU-01/default-1706x938.png`](MENU-01/default-1706x938.png) remains a reference for unchanged retro controls, density, statistics table, and footer placement only.
The historical 1706 by 938 artifact is not evidence for the consolidated Persons list or the current scaled-background presentation.
...
The supplied PR #60 Teams and non-Team artifacts conformed visually for that refinement and are now historical.
The localized PR #62 refinement affects both existing `MENU-01` wireframes.
The PR #62 artifacts replace the PR #60 artifacts because the visible Persons list height, person-name row position, action-button geometry, and batch controller-detection caption changed.
The matrix still requires exactly one representative screenshot for each existing wireframe.
The refreshed Teams and non-Team artifacts conform visually and have complete required provenance.
The focused UX assessment used exact pushed PR #62 head `316c43f99e2f0178b8581012393fd7ecd5353776` and fixed specification baseline `88b72a6`.
The localized PR #65 placement refinement affects both existing `MENU-01` wireframes.
The PR #62 artifacts are historical because they show `<<` in Persons.
The PR #65 Teams artifact replaces `SS-024`.
The PR #65 non-Team artifact replaces `SS-001`.
The matrix still requires exactly one representative screenshot for each wireframe.
...
- Freshness note: production implementation is unchanged between the production source SHA, capture source state, and current head.
...
- Final evidence status: `SS-001` and `SS-024` conform at final PR #65 head `c2d72fb4d2c2bb27e6e525d88368543d710eb5b9`.
